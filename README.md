...
在 App 的「模型」页点击某一行的「查看日志」，每次仅拉取该模型最近 50 条记录，可筛选业务调用、手动测试、自动恢复及失败记录，也可继续加载更早记录。关闭窗口后不再拉取。

「业务成功率」只统计正常请求；手动测试的简短请求成功，不代表长对话、工具调用或流式请求也成功。日志显示耗时、HTTP 状态、安全原因码、输入规模和下一候选模型；降级前的失败不会再被最终成功掩盖。客户端主动停止单独标记，不计为模型失败。

服务器保留最近 50,000 条模型调用摘要，不保存对话正文、请求头、密钥或上游原始错误体。详细日志从升级网关后开始记录，旧版本没有留下的失败原因无法补回。App 和 VPS 网关均需升级到 v0.1.3 或更新版本。
