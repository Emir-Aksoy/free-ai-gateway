...
**轻量、易用的个人 API 管理工具**。通过 Mac App 管理本机或自己的 VPS，把不同服务商统一到一个 OpenAI 兼容 `/v1` 地址。目前侧重**免费 API 的聚合调用**，使用你自己的 Provider 账号与密钥。

[免费 API 目录与接入步骤](docs/free-api-catalog.md) · [同类项目对比](docs/comparison.md) · [下载 App](https://github.com/Emir-Aksoy/free-ai-gateway/releases/latest)

## v1.3：精简接入助手

助手只做 Provider / 模型发现与配置文字指引。每次提问由 App 先读取本项目 GitHub 的[免费 API 目录](docs/free-api-catalog.json)，再把相关目录条目和内置软件使用手册交给当前实例 thinking 列表中最优先可用的模型；失败按该列表降级。普通客户端的原有评分/排序策略不变，界面显示实际模型、耗时和降级摘要。

这种固定流程不要求小模型自己选择联网工具。目录、Mac / VPS 设置、Provider 新增与测试、模型排序、密钥、日志、双端同步、用量和 Agent 接入说明均有内置指引。没有可用模型时也能浏览目录和手册。先配置 Provider 与 thinking 调用链，启动网关并保留一个启用的客户端密钥后即可提问；App 和 VPS 需升级到v1.3。

助手不自动改配置、不执行命令、不索取密钥，没有全网爬虫或常驻搜索进程。用户在现有表单中操作。每次提问读取 GitHub，失败明确使用带日期的内置快照；对话仅在当前页面内保留，历史和回复有大小上限。模型调用计入网关已有用量；停止等待不保证撤销已发送的上游请求。

目录区分持续免费额度、试用与待核查，注明官方来源及日期。试用赠金不能当成持续免费；未知定价不猜测，当前账号资格与实际测试结果仍需核实。维护办法见[目录说明](docs/free-api-catalog.md#维护方式)。
